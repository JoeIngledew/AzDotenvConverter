...
impl TryFrom<&str> for AzOutputItem {
    type Error = ConversionError;
...
mod test {
    ...
    #[quickcheck_macros::quickcheck]
    fn try_from_str_slice_valid_input_should_return_correct_output(s1: SafeName, s2: SafeValue) {
        let s = format!("{}={}", &s1.0, &s2.0);
        let str = s.as_str();
        let result = AzOutputItem::try_from(str).unwrap();
        let expected = AzOutputItem::new(&s1.0, &s2.0);
        assert_eq!(result, expected);
    ...
    #[quickcheck_macros::quickcheck]
    fn try_from_str_slice_invalid_input_should_return_correct_output(s1: SafeName, s2: SafeValue) {
        let s = format!("{}{}", &s1.0, &s2.0);
        let str = s.as_str();
        let result = AzOutputItem::try_from(str);
        assert!(result.is_err_and(|e| matches!(e, ConversionError::Generic(_))))
